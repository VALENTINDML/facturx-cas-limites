# PDF-005 — Métadonnées XMP Factur-X absentes

| | |
|---|---|
| Catégorie | structure PDF |
| Ce que le cas teste | Conteneur PDF/A-3 ou spécification Factur-X |
| Règle | Factur-X 1.0.07 — extension XMP obligatoire (hors schematron) |
...
(PDF-005) est un identifiant du corpus, pas un code de norme.

## Le défaut

Aucune métadonnée XMP ne déclare le profil ni le nom du fichier. Contrôle au niveau du conteneur PDF.

## Pourquoi une implémentation le rate

Le XML reste extractible : une implémentation qui ne lit que la pièce jointe ne verra jamais le problème.
